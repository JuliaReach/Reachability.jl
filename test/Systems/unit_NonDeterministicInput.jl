...
input_remains_constant = state.sf.center == c && state.sf.radius == r
@test input_remains_constant
for i in 1:5
    state = next(input_iter, state)
    input_remains_constant = input_remains_constant && state.sf.center == c && state.sf.radius == r
end
...
state = start(input_iter)
input_is_varying = state.sf.center == c[1] && state.sf.radius == r[1]
@test input_is_varying
for i in 2:3
    state = next(input_iter, state)
    input_is_varying = input_is_varying && state.sf.center == c[i] && state.sf.radius == r[i]
end
@test input_is_varying
